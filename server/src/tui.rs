...
impl ServerTui {
    ...
    fn update_metrics(&mut self) {
        ...
        if self.connection_history.len() > MAX_HISTORY_SIZE {
    ...
    fn render_ui(&self, f: &mut Frame) {
        ...
        self.render_server_stats(f, chunks[1]);
    ...
    fn render_header(&self, f: &mut Frame, area: Rect) {
        ...
        let header = Paragraph::new(vec![
            Line::from(vec![
                Span::styled("NPT Server Dashboard", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::raw("  |  "),
                Span::styled(format!("Uptime: {:02}:{:02}:{:02}", 
    ...
    fn render_server_stats(&self, f: &mut Frame, area: Rect) {
        let chunks = Layout::default()
        ...
        let tcp_gauge = Gauge::default()
            .block(Block::default().borders(Borders::ALL).title("TCP Connections"))
            .gauge_style(Style::default().fg(Color::Blue))
            .percent(std::cmp::min(tcp_connections * 100 / 500, 100) as u16)
            .label(format!("{}", tcp_connections));

        f.render_widget(tcp_gauge, chunks[0]);

        let udp_connections = self.metrics.get_active_udp_connections();
        let udp_gauge = Gauge::default()
            .block(Block::default().borders(Borders::ALL).title("UDP Sessions"))
            .gauge_style(Style::default().fg(Color::Green))
            .percent(std::cmp::min(udp_connections * 100 / 500, 100) as u16)
            .label(format!("{}", udp_connections));
    ...
    fn render_charts(&self, f: &mut Frame, area: Rect) {
        ...
        self.render_connection_chart(f, left_chunks[0]);
        self.render_throughput_sparkline(f, left_chunks[1]);
        self.render_latency_chart(f, right_chunks[0]);
        self.render_error_chart(f, right_chunks[1]);
    }

    fn render_connection_chart(&self, f: &mut Frame, area: Rect) {
        if self.connection_history.is_empty() {
            let empty_chart = Paragraph::new("No connection data available")
        ...
        let max_time = self.connection_history.last().map(|x| x.0).unwrap_or(1.0);
    ...
    fn render_throughput_sparkline(&self, f: &mut Frame, area: Rect) {
        ...
        if throughput_data.is_empty() {
            let empty_sparkline = Paragraph::new("No throughput data")
    ...
    fn render_latency_chart(&self, f: &mut Frame, area: Rect) {
        if self.latency_history.is_empty() {
            let empty_chart = Paragraph::new("No latency data available")
                .block(Block::default().borders(Borders::ALL).title("Response Latency (ms)"))
                .style(Style::default().fg(Color::Gray));
            f.render_widget(empty_chart, area);
            return;
        }
